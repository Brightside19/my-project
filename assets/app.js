...
function renderMain() {
  ...
  groupsToShow.forEach(g => {
    ...
    new Sortable(body, {
      group: 'links',
      animation: 150,
      handle: isMobile ? '.favicon' : undefined, // ← вот это добавляем
      onEnd: async (evt) => {
        const item = evt.item;
...
function applySearchFilter() {
  ...
  // Прячем группы без подходящих карточек
  document.querySelectorAll('.group-block').forEach(group => {
    const visibleCards = group.querySelector('.card:not([style*="display: none"])');
